...
//
//  std::fstream train_file("data/train_images.txt");
...
//  std::ofstream out_label_file("data/out_prediction.txt");
...
//          blurImage, otsuImage, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU
//      );
//
...
//      cv::Canny(blurImage, edgesImage, low_thresh_val, high_thresh_val);
...
//  //evaluation
//  std::fstream detector_file("data/out_prediction.txt");
//  std::fstream groundtruth_file("data/groundtruth.txt");
...
//  detectorOutputFile.seekg(0, std::ios::beg);
//  groundTruthFile.clear();
//  groundTruthFile.seekg(0, std::ios::beg);
//
//  std::ofstream moveShellFile("move.sh", std::ofstream::trunc);
//
//  int counter = 0;
//  while (true) {
//
...
//      cv::Canny(blurImage, edgesImage, threshold1, threshold2);
//
//      std::stringstream blurPath;
//      blurPath << "./output/false/positives/" << counter - 1 << "_blur.jpg";
//
//
//      std::stringstream edgesPath;
